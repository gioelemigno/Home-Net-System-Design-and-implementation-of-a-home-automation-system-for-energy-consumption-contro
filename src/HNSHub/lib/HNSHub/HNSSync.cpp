...
res_t HNSSync_sync(address_t* address){
  ...
  if(res_r == 5 && packetRX.type==SEND_MODEL){
    uint16_t model = ((packetRX.data[2] << 8) | ((uint8_t)packetRX.data[1]));
    uint16_t uid = ((packetRX.data[4] << 8) | ((uint8_t)packetRX.data[3]));
    #if defined(PRINT) && defined(DEBUG_SYNC)
    ...
    if(res==-1){
      printf("errore HNSDatabase_getNewAddress \n");
      return -1;
    }
    ...
    res_t ran = HNSDatabase_add2(model, &address_newDevice, uid);
    if(ran==-1){
      printf("errore \n");
  ...
  *address=address_newDevice;
  return 0;
}

res_t bak_HNSSync_sync(address_t* address){
