...
res_t HNSMQTT_init(const char* host){
 ...
 // mosquitto_username_pw_set(mosq, "shelly", "shelly_pass");
  ...
  std::string sub_topic = "custom/HNSSocketPowerFF2354FF/0/switch/commands"; //prefix + "#";
...
//--------------- DELET IT ----------------------------------
    ...
    void bak_message_callback(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message)
    {
      #if defined(PRINT) && defined(DEBUG_MQTT)
      ...
      std::string device;
      std::string command;
      std::string msg_topic = std::string(message->topic); 

      int res =  splitTopic(msg_topic, device, command);

      if(res==0){
        #if defined(PRINT) && defined(DEBUG_MQTT)
        msg = std::string("Device=");
        msg+=std::string( device.c_str());
        msg+=" Command=" + std::string(command.c_str());
        Utils.printDebug(__func__, msg.c_str());
        #endif
        ...
        #if defined(PRINT) && defined(DEBUG_MQTT)
        Utils.printDebug(__func__, "Error in splitTopic()");
        #endif
        exit(EXIT_FAILURE);
      }
      ...
      //printf("blocco\n");
        ...
        std::string echo_topic = std::string(ECHO_PREFIX) + msg_topic;
      ...
      HNSDatabase_entry entry = HNSDatabase_deviceList[address];
      res_t result = entry.device->runCommand(command, std::string((char*)message->payload));
      if(result==-1){
        info_error("");
      }
  ...
  res_t bak_HNSMQTT_publishDevice(address_t address){
    ...
    #if defined(PRINT) && defined(DEBUG_MQTT)
    msg = std::string("Address device=");
    msg += std::to_string(address);
    Utils.printDebug(__func__, msg.c_str());
    #endif
    
    double time = Utils.getTime_us();
    double elapsed = time-entry.device->last_upload; 

    if(elapsed < device->update_min_period_us){
      //too early to upload (even with force)
      return -1;
    }
    if(device->force_upload){
      #if defined(PRINT) && defined(DEBUG_MQTT)
      Utils.printDebug(__func__, "Force_upload==true");
      #endif
    }
    else{ //device->force_upload=false
      //double elapsed = time-entry.device->last_upload; 
      if( elapsed < device->update_period_us){
        #if defined(PRINT) && defined(DEBUG_MQTT)
        msg = std::string("Too early to upload");
        msg+=std::string(" remain=");
        msg+=std::to_string((device->update_period_us - elapsed)*1E-06);
        msg+="s";
        Utils.printDebug(__func__, msg.c_str());
        #endif
    ...
    //publish only topic with mqtt.force_publish
    if(device->force_upload==true){ 
      for(unsigned j=0; j<entry.device->number_topics; j++){
        if(entry.device->MQTT_message[j].forceMQTTPublish==true){
          HNSMQTT_publish(entry.device->MQTT_message[j].topic,entry.device->MQTT_message[j].payload);
          entry.device->MQTT_message[j].forceMQTTPublish = false;
        }
      }
    }
    else{ //refresh all
    ...
    device->last_upload=time;
    device->force_upload=false;
    return 0;
  }
  ...
  res_t HNSMQTT_publishAll(){
    ...
    for(uint16_t i=0; i<HNSDatabase_deviceList_index; i++){
      ...
      for(unsigned j=0; j<entry.device->number_topics; j++){
        HNSMQTT_publish(entry.device->MQTT_message[j].topic,entry.device->MQTT_message[j].payload);
        //HNSMQTT_publish(std::string("temp_0"), std::string("30.0C°"));
      }
    }
    */
    printf("NO PUB ALL\n");
...
static inline std::string buildJSONString(AnnounceMessage_struct announce){
  ...
  result += JSONLine_string("command_topic", announce.command_topic);
